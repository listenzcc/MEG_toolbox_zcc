...
results_folder = os.path.join('Results_generalize')
...
for uid in uids:
    ...
    selects = select_events(test_epochs)
    selected_test_epochs = test_epochs[selects]

    display(train_epochs, selected_train_epochs,
            test_epochs, selected_test_epochs)

    # Get X and y
    train_X, train_y = get_X_y(selected_train_epochs)
    test_X, test_y = get_X_y(selected_test_epochs)
    ...
    generalize_estimator = mne.decoding.GeneralizingEstimator(
        base_estimator=clf, n_jobs=32)

    # Fit
    generalize_estimator.fit(X=train_X, y=train_y)

    # Predict
    generalize_pred_y = generalize_estimator.predict(X=test_X)

    # Save results
    results = dict(generalize_pred_y=generalize_pred_y,
                   test_y=test_y,
                   times=times)
