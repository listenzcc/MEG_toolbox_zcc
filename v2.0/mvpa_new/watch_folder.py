...
def parse_dict(report, table, name, method):
    groups = ['1', '2', '4', 'macro avg', 'weighted avg']
...
for folder in ['SVM_xdawn_1']:
    results_dir = os.path.join('.', folder)

    for name in os.listdir(results_dir):
        print(name)
        # if not name == 'MEG_S01.json':
        # continue
...
plotly.offline.iplot({
    'data': [
        go.Scatter(y=y_true, name='True'),
        go.Scatter(y=-4 + y_pred, name='Pred'),
        go.Scatter(y=7 + y_true-y_pred, name='Diff'),
    ],
    'layout': go.Layout(title='MVPA')
})
# targets = np.where(pred['y_test'] == 1)[0]
# padding = 5
# for t in targets:
#     print(t, pred['y_pred'][t], pred['y_pred'][t-padding:t+padding+1])
